...
namespace ReplayEngine::Components
{
    ...
    void UIShapeComponent::NormalizePathArrays()
    {
    ...
    void UIShapeComponent::SetPathPointCount(int count)
    {
    ...
    void UIShapeComponent::OnPropertyChanged(const char*)
    {
        NormalizePathArrays();
        RebuildDynamicProperties();
    }
    ...
    const std::vector<Reflection::PropertyDesc>*
        UIShapeComponent::DynamicProperties() const noexcept
    {
        const_cast<UIShapeComponent*>(this)->NormalizePathArrays();
        RebuildDynamicProperties();
        return dynamic_properties_.empty() ? nullptr : &dynamic_properties_;
    }

    void UIShapeComponent::RebuildDynamicProperties() const
    {
        ...
        for (std::size_t i = 0; i < path_points.size(); ++i)
        {
            ...
            const auto add_vector = [&](const char* field, const char* display,
                std::vector<DirectX::XMFLOAT2> UIShapeComponent::* member)
            {
                ...
                {
                    const auto& shape = static_cast<const UIShapeComponent&>(component);
                ...
                {
                    auto& shape = static_cast<UIShapeComponent&>(component);
            ...
            add_vector("position", "位置", &UIShapeComponent::path_points);
            add_vector("in_handle", "入力ハンドル", &UIShapeComponent::path_in_handles);
            add_vector("out_handle", "出力ハンドル", &UIShapeComponent::path_out_handles);
        }
    }
}
