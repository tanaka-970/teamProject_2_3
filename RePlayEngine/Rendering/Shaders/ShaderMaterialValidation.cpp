...
int RunShaderMaterialValidation(){
 ...
 const fs::path migrated=dir/"migrated.replaymaterial"; c.Expect(MaterialAsset::Save(old,migrated,error),"migrated save"); std::ifstream check(migrated); std::string text((std::istreambuf_iterator<char>(check)),{}); c.Expect(text.find("REPLAY_MATERIAL 4")!=std::string::npos,"save always v4");
