...
int RunShaderMaterialValidation(){
 ...
 std::string error; const fs::path v3=dir/"v3.replaymaterial"; c.Expect(MaterialAsset::Save(source,v3,error),"v3 save"); MaterialAsset loaded; c.Expect(MaterialAsset::Load(v3,loaded,error),"v3 load"); c.Expect(loaded.shader_guid==BuiltInShaders::Toon.ToString(),"shader guid roundtrip"); c.Expect(loaded.properties.Find("prop.BaseColor")!=nullptr,"legacy BaseColor migrated into properties"); const auto* unknown=loaded.properties.Find("prop.UnknownFutureValue"); c.Expect(unknown && unknown->AsString()=="keep-me","unknown property preserved"); c.Expect(loaded.roughness>0.71f && loaded.roughness<0.73f,"legacy field synchronized from property bag");
 const fs::path v1=dir/"v1.replaymaterial"; { std::ofstream s(v1); s<<"REPLAY_MATERIAL 1\nBASE_COLOR 1 0 0 1\nBASE_COLOR_TEXTURE \"\"\nNORMAL_TEXTURE \"\"\nMETALLIC 0.25\nMETALLIC_TEXTURE \"\"\nROUGHNESS 0.5\nROUGHNESS_TEXTURE \"\"\nEMISSIVE 0 0 0 0\nEMISSIVE_TEXTURE \"\"\nAMBIENT_OCCLUSION 1\nAMBIENT_OCCLUSION_TEXTURE \"\"\nALPHA 0 0.5\nDOUBLE_SIDED 0\nSHADING_MODEL 1\nEND_MATERIAL\n"; }
 MaterialAsset old; c.Expect(MaterialAsset::Load(v1,old,error),"v1 load"); c.Expect(old.shader_guid==BuiltInShaders::Pbr.ToString(),"v1 shading_model to shader guid"); c.Expect(old.properties.Find("prop.Metallic")!=nullptr,"v1 fixed fields migrated");
 const fs::path migrated=dir/"migrated.replaymaterial"; c.Expect(MaterialAsset::Save(old,migrated,error),"migrated save"); std::ifstream check(migrated); std::string text((std::istreambuf_iterator<char>(check)),{}); c.Expect(text.find("REPLAY_MATERIAL 3")!=std::string::npos,"save always v3");
