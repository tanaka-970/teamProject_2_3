...
framebuffer::framebuffer(ID3D11Device* device, uint32_t width, uint32_t height)
{
	...
	hr = device->CreateTexture2D(&texture2d_desc, 0,
		render_target_buffer.GetAddressOf());
	...
	float r, float g, float b, float a, float depth)
{
	...
	immediate_context->ClearDepthStencilView(depth_stencil_view.Get(), D3D11_CLEAR_DEPTH | D3D11_CLEAR_STENCIL, depth, 0);
}
void framebuffer::activate(ID3D11DeviceContext* immediate_context)
{
	...
	immediate_context->OMGetRenderTargets(
		1,
		cached_render_target_view.ReleaseAndGetAddressOf(),
