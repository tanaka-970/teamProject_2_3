...
# SPARK CHASE を単体 EXE として組み立てて起動する。
...
$output = Join-Path $root 'Saved/SparkChase'
...
Copy-Item -LiteralPath $engine -Destination (Join-Path $output 'SparkChase.exe') -Force
Get-ChildItem -LiteralPath (Split-Path $engine) -Filter '*.dll' | Copy-Item -Destination $output -Force

foreach ($relative in @('resources/Game/SparkChase', 'Shader', 'ThirdParty/DXC/bin/x64',
...
REPLAY_GAME 1
NAME SPARK CHASE
STARTUP_SCENE resources/Game/SparkChase/SparkChase.replayscene
WINDOW 1600 900
FULLSCREEN 0
'@
Write-Utf8 (Join-Path $output 'SparkChase.replaygame') $game

$arguments = @('--game')
if ($Frames -gt 0) { $arguments = @('--smoke-test', "$Frames", '--game') }

if ($Headless) {
    $process = Start-Process -FilePath (Join-Path $output 'SparkChase.exe') -WorkingDirectory $output `
...
Write-Output "SPARK CHASE PID: $($process.Id)"
